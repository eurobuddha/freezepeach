...
public final class ImageLoader {
    ...
    public static void loadFull(final android.app.Activity act, final String url, final ImageView iv, int fallbackRes) {
    ...
    public static void loadOver(final android.app.Activity act, final String url, final ImageView iv, final Runnable onLoaded) {
        iv.setTag(url);
    ...
    private static void load(final android.app.Activity act, final String url, final ImageView iv, int fallbackRes, final int reqPx) {
        iv.setTag(url);
    ...
    private static byte[] fetch(String url) throws Exception {
        ...
        HttpURLConnection con = openSafe(f, 8000, 15000);   // manual redirects + per-hop SSRF revalidation
        ...
        try {
            for (java.net.InetAddress a : java.net.InetAddress.getAllByName(host)) {
                if (a.isLoopbackAddress() || a.isAnyLocalAddress() || a.isLinkLocalAddress()
                        || a.isSiteLocalAddress() || a.isMulticastAddress() || isPrivateExtra(a))
                    return true;
            }
        } catch (Exception e) { return true; }
        return false;
    }
    /** Ranges Java's isSiteLocalAddress misses: IPv6 ULA fc00::/7 and IPv4 CGNAT 100.64.0.0/10 (incl. IPv4-mapped). */
    private static boolean isPrivateExtra(java.net.InetAddress a) {
        byte[] b = a.getAddress();
        if (b.length == 16) {
            if ((b[0] & 0xFE) == 0xFC) return true;                       // fc00::/7 unique-local
            boolean mapped = (b[10] & 0xFF) == 0xFF && (b[11] & 0xFF) == 0xFF;
            for (int i = 0; i < 10 && mapped; i++) if (b[i] != 0) mapped = false;
            if (mapped) return cgnat(b[12], b[13]);                       // ::ffff:100.64.x.x
        } else if (b.length == 4) {
            return cgnat(b[0], b[1]);
        }
        return false;
    }
    private static boolean cgnat(byte b0, byte b1) {
        int o0 = b0 & 0xFF, o1 = b1 & 0xFF;
        return o0 == 100 && o1 >= 64 && o1 <= 127;                        // 100.64.0.0/10
    }

    /** Open a connection following redirects MANUALLY, re-validating the host on EVERY hop — defeats the
     *  redirect + DNS-rebinding SSRF bypass. Returns a connected non-redirect connection, or null if any hop
     *  resolves to a blocked host / non-http(s) scheme / too many redirects. Caller reads + disconnects. */
    static HttpURLConnection openSafe(String url, int connectTimeout, int readTimeout) throws Exception {
        String cur = url;
        for (int hop = 0; hop < 5; hop++) {
            URL u = new URL(cur);
            String proto = u.getProtocol() == null ? "" : u.getProtocol().toLowerCase();
            if (!proto.equals("http") && !proto.equals("https")) return null;
            if (isBlockedHost(u.getHost())) return null;
            HttpURLConnection con = (HttpURLConnection) u.openConnection();
            con.setInstanceFollowRedirects(false);
            con.setConnectTimeout(connectTimeout);
            con.setReadTimeout(readTimeout);
            con.setRequestProperty("User-Agent", "FreezePeach");
            int code = con.getResponseCode();
            if (code >= 300 && code < 400) {
                String loc = con.getHeaderField("Location");
                con.disconnect();
                if (loc == null) return null;
                cur = new URL(u, loc).toString();                        // resolve relative Location, re-check next loop
                continue;
            }
            return con;
        }
